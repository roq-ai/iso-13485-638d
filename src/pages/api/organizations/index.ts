...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.iso_certificate?.length > 0) {
      const create_iso_certificate = body.iso_certificate;
      body.iso_certificate = {
        create: create_iso_certificate,
      };
    } else {
      delete body.iso_certificate;
    }
